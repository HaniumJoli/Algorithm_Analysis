...
class Node // a node represents a state (includes worker, job, assigned jobs, path cost, promising cost, parent node)
{
public:
    int parent; // parent node
    int nodeNo;
    int pathCost; // total cost so far.
    int promisingCost; // least promising cost. (path cost + cost of least promising assignment)
    ...
    Node(int w_id, int j_id,vector<bool>_assigned, int node_num,int par)
    {
        ...
        jobID = j_id;
        assigned = _assigned;
        if(jobID+1) assigned[jobID] = true; // if jobID is not -1, then the job has been assigned.
        parent = par;
        nodeNo = node_num;
    }
